...
// ── SUPABASE ─────────────────────────────────────────────
// ── DB CONFIG ────────────────────────────────────────────
// All requests go through the edge function proxy (fixes CORS for GitHub Pages
// and any other hosted domain). The proxy uses the service role key server-side.
...
const PROXY='https://nyzkkqqjnkctcmxoirdj.supabase.co/functions/v1/data';
const SBH_READ ={'apikey':SB_KEY,'Authorization':'Bearer '+SB_KEY,'Content-Type':'application/json'};
const SBH_WRITE={'Content-Type':'application/json'}; // proxy handles auth server-side
const SBH=SBH_READ;
...
window.oaDB={
  async load(){
    ...
    try{
      const [cr,rr]=await Promise.all([
        fetch(`${PROXY}/companies`,{headers:SBH_READ}),
        fetch(`${PROXY}/contacts`,{headers:SBH_READ}),
      ]);
      if(!cr.ok||!rr.ok){
  ...
  async saveCompany(r){
    try{
      ...
      await fetch(`${PROXY}/companies`,{method:'POST',headers:SBH_WRITE,body:JSON.stringify(body)});
  ...
  async saveContact(r){
    try{
      ...
      await fetch(`${PROXY}/contacts`,{method:'POST',headers:SBH_WRITE,body:JSON.stringify(body)});
