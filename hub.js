'use strict';
/* ═══════════════════════════════════════════════
   onAudience Sales Intelligence Hub — hub.js v2
   Multi-select · Battleground-first · MEESEEKS
═══════════════════════════════════════════════ */
...
function classify(n) {
  ...
  if(s.includes('no outreach')||s.includes('no fit')||s.includes('no business')||s.includes('internal')||s.includes('closed')||s.includes('unwanted')) return 'nogo';
...
function contactScore(ct) {
  ...
  return {fields:results, pct:Math.round(results.filter(r=>r.has).length/fields.length*100)};
}
function companyScore(c) {
  ...
  return {fields:results, pct:Math.round(results.filter(r=>r.has).length/results.length*100)};
}
function contactsForCompany(name) {
  const n=name.toLowerCase();
  return contacts.filter(ct=>{const cn=(ct.company_name||'').toLowerCase();return cn===n||cn.includes(n)||n.includes(cn);});
}

/* ── Multi-select state ── */
// Maps: { uid (name or contact id), kind ('company'|'contact'), payload }
let selection = new Map(); // uid -> {kind, name, meta, company, category, region, icp, note, contacts}

function toggleSelect(uid, kind, payload, event) {
  event && event.stopPropagation();
  if (selection.has(uid)) selection.delete(uid);
  else selection.set(uid, { kind, ...payload });
  updateSelectionBar();
  renderList();
  if (leftTab === 'contacts') renderContactsList();
}
...
function updateSelectionBar() {
  ...
  const companies = [...selection.values()].filter(i=>i.kind==='company').length;
  const contacts = [...selection.values()].filter(i=>i.kind==='contact').length;
  if (selection.size === 0) { bar.classList.remove('vis'); return; }
  bar.classList.add('vis');
  const parts = [];
  if (companies) parts.push(`${companies} co.`);
  if (contacts) parts.push(`${contacts} ct.`);
  count.textContent = `${selection.size} selected (${parts.join(' + ')})`;
}

function getSelectionPayload() {
  return [...selection.values()].map(item => {
    const c = companies.find(co => co.name === item.name);
    const cts = c ? contactsForCompany(c.name) : [];
    return {
      kind: item.kind,
      name: item.name,
      meta: item.meta || '',
      ...
      region: item.region || c?.region || '',
      icp: item.icp || c?.icp || null,
      note: item.note || c?.note || '',
      website: c?.website || '',
      contacts: cts.map(ct => ({ name: ct.full_name, title: ct.title, email: ct.email, linkedin: ct.linkedin_url })),
    };
  });
}

/* ── Send selection to panels ── */
function sendSelectionToBattleground() {
  ...
  const send = () => {
    try { iframe.contentWindow.postMessage({ type: 'ADD_SELECTION', payload }, '*'); } catch(e) {}
  };
...
function sendSelectionToMeeseeks() {
  if (!selection.size) return;
  const payload = getSelectionPayload();
  const meeseeksPayload = payload.map(item => ({
  ...
  openMeeseeks(meeseeksPayload.length === 1 ? meeseeksPayload[0] : meeseeksPayload);
}
...
function renderList() {
  ...
  if (!filtered.length) { list.innerHTML='<div style="padding:24px;text-align:center;font-family:IBM Plex Mono,monospace;font-size:10px;color:var(--t3);text-transform:uppercase">No results</div>'; return; }
  list.innerHTML = filtered.map(c => {
    const [bg,fg] = av(c.name);
    const isSel = selection.has(c.name);
    const isOpen = c.name === selectedName;
    ...
    return `<div class="co-row${isOpen?' selected':''}${isSel?' cb-selected':''}"
      onclick="openDetail('${enc}')"
      ondragstart="startDrag(event,'${enc}','company')" draggable="true"
      oncontextmenu="showCtx(event,'${enc}');return false">
      <div class="co-cb${isSel?' checked':''}" onclick="toggleSelect('${enc}','company',{name:'${esc(c.name)}',meta:'${esc(c.note)}',category:'${esc(c.category||'')}',region:'${esc(c.region||'')}',icp:${c.icp||0},note:'${esc(c.note)}'},event)" title="Select"></div>
...
function renderContactsList() {
  ...
  if (!list.length) { el.innerHTML='<div style="padding:24px;text-align:center;font-family:IBM Plex Mono,monospace;font-size:10px;color:var(--t3)">No contacts</div>'; return; }
  el.innerHTML = list.map(ct => {
    const {pct}=contactScore(ct);
    const dotCls=pct>=75?'full':pct>=40?'partial':'empty';
    const uid = ct.id||ct.full_name||'';
    const enc=encodeURIComponent(uid);
    const isSel = selection.has(uid);
    return `<div class="ct-row${isSel?' cb-selected':''}"
      draggable="true" ondragstart="startDrag(event,'${enc}','contact')"
      onclick="openContactDrawer('${enc}')">
      <div class="co-cb${isSel?' checked':''}" onclick="toggleSelect('${enc}','contact',{name:'${esc(ct.full_name||'')}',meta:'${esc(ct.title||'')}',company:'${esc(ct.company_name||'')}'},event)" title="Select"></div>
...
function openContactDrawer(enc) {
  ...
  document.getElementById('drawerBody').innerHTML=`
    ...
    <div style="display:flex;gap:5px;flex-wrap:wrap">
      <button class="btn p sm" onclick="sendSingleToBattleground('contact','${esc(ct.full_name||'')}','${esc(ct.title||'')}','${esc(ct.company_name||'')}')">⚔ Battleground</button>
...
function sendSingleToBattleground(kind, name, meta, company) {
  const c = companies.find(co=>co.name===name)||companies.find(co=>co.name===(company||''));
  const cts = c ? contactsForCompany(c.name) : [];
  const payload = [{ kind, name, meta, company: company||name,
    category: c?.category||'', region: c?.region||'', icp: c?.icp||null,
    note: c?.note||'', contacts: cts.map(ct=>({name:ct.full_name,title:ct.title,email:ct.email})) }];
  switchPageTab('battleground');
  const iframe = document.getElementById('battleground-iframe');
  const send = () => { try { iframe.contentWindow.postMessage({type:'ADD_SELECTION',payload},  '*'); } catch(e) {} };
  if (panelLoaded.battleground && iframe.contentDocument?.readyState==='complete') send();
  else iframe.addEventListener('load', send, {once:true});
}
...
function renderContactCard(ct) {
  ...
  return `<div class="ct-card">
    <div class="ct-top">
      ...
      <button class="btn sm" onclick="sendSingleToBattleground('contact','${esc(ct.full_name||'')}','${esc(ct.title||'')}','${esc(ct.company_name||'')}')">⚔</button>
    </div>
  ...
  document.getElementById('centerPanel').innerHTML=`<div class="co-detail">
    <div class="det-header">
      ...
      <div class="det-actions">
        ...
        <button class="btn sm" onclick="sendSingleToBattleground('company','${esc(name)}','${esc(c.note)}','${esc(name)}')">⚔ Battleground</button>
      </div>
    </div>
    <div class="section">
    ...
    <div class="section">
      ...
      <div class="sec-body"><div class="act-row">
        ...
        <button class="btn sm p" onclick="sendSingleToBattleground('company','${esc(name)}','${esc(c.note)}','${esc(name)}')">⚔ Battleground</button>
        <button class="btn sm" onclick="openClaude('Find companies similar to ${esc(name)} — top 10 by ICP fit')">🔗 Similar</button>
...
function resetCenter() {
  document.getElementById('centerPanel').innerHTML=`<div class="qa-grid">
    ...
    <div class="qa-card" onclick="openClaude('Find new DSP, SSP and media agency prospects for onAudience in EMEA')"><div class="qa-icon">🎯</div><div class="qa-title">Find Prospects</div><div class="qa-desc">EMEA focus</div></div>
    <div class="qa-card" onclick="switchPageTab('battleground')"><div class="qa-icon">⚔️</div><div class="qa-title">Battleground</div><div class="qa-desc">Select targets → plan outreach</div></div>
    <div class="qa-card" onclick="switchPageTab('meeseeks')"><div class="qa-icon">✉️</div><div class="qa-title">MEESEEKS</div><div class="qa-desc">Compose outreach email</div></div>
    <div class="qa-card" onclick="openClaude('Re-engage dormant onAudience prospects — not contacted in 60+ days')"><div class="qa-icon">♻️</div><div class="qa-title">Re-engage</div><div class="qa-desc">Dormant prospects</div></div>
    <div class="qa-card" onclick="openClaude('Top 5 highest-priority onAudience prospects this week')"><div class="qa-icon">⭐</div><div class="qa-title">Top Picks</div><div class="qa-desc">This week</div></div>
  </div>`;
}
...
function showCtx(e, enc) {
  ...
  const actions=[
    ...
    {icon:'⚔️',text:'Add to Battleground',    fn:()=>sendSingleToBattleground('company',name,c?.note||'',name)},
    {icon:'🔗',text:'Find similar companies', fn:()=>openClaude(`Find companies similar to ${name} — top 10`)},
...
/* ── Page tabs ── */
const PANEL_SRCS = { battleground:'battleground-panel.html', meeseeks:'meeseeks-composer.html' };
let panelLoaded = {};
...
function switchPageTab(tab) {
  ...
  if (PANEL_SRCS[tab]) {
    ...
    if (!panelLoaded[tab]) {
      ...
      iframe.addEventListener('load',()=>syncTheme(iframe),{once:true});
    }
  }
}
...
window.addEventListener('message', e => {
  ...
  if (d.type==='LAUNCH_MEESEEKS') openMeeseeks(d.payload);
...
function startDrag(e, enc, kind) {
  const name=decodeURIComponent(enc);
  const c=kind==='company'?companies.find(co=>co.name===name):null;
  const ct=kind==='contact'?contacts.find(co=>(co.id||co.full_name||'')===name||co.full_name===name):null;
  const cts=c?contactsForCompany(c.name):[];
  const data = {
    kind, name,
    meta: c?.note||ct?.title||'',
    company: ct?.company_name||name,
    category: c?.category||'', region: c?.region||'',
    icp: c?.icp||null, note: c?.note||'',
    contacts: cts.map(co=>({name:co.full_name,title:co.title,email:co.email}))
  };
...
function setupTabDropZones() {
  ['ptab-battleground','ptab-meeseeks'].forEach(id=>{
    ...
    el.addEventListener('drop',e=>{
      ...
      if(id==='ptab-battleground') {
        switchPageTab('battleground');
        const iframe=document.getElementById('battleground-iframe');
        const send=()=>{try{iframe.contentWindow.postMessage({type:'ADD_SELECTION',payload:[data]},'*');}catch(e){}};
        if(panelLoaded.battleground&&iframe.contentDocument?.readyState==='complete') send();
        else iframe.addEventListener('load',send,{once:true});
      } else {
        openMeeseeks({company:data.company||data.name,contactName:data.kind==='contact'?data.name:'',contactTitle:data.meta,description:data.note});
      }
    });
  });
}
...
function toggleTheme(){
  ...
  Object.keys(PANEL_SRCS).forEach(tab=>{if(panelLoaded[tab])syncTheme(document.getElementById(tab+'-iframe'));});
}
