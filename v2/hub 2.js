...
function renderRelGraph(){
  ...
  if(_relDepth===2){
    ...
    S.allRelations.forEach(r=>{
      const fIn=hop1Ids.has(r.from_company)&&r.from_company!==centerSlug;
      const tIn=hop1Ids.has(r.to_company)&&r.to_company!==centerSlug;
  ...
  edges.forEach(e=>{
    ...
    /* edge label — only for hop-1 and path edges */
    if(e.hop===1||isPath){
